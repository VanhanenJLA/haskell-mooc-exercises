...
instance Functor TwoList where
  ...
  fmap f (TwoNode a b list) = TwoNode (f a) (f b) (fmap f list)
...
inBoth f g = foldr (\x acc -> if x `elem` g then x : acc else acc) [] f
...
instance Foldable List where
  ...
  foldr f acc (LNode a list) = f a (foldr f acc list)
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode a b list) = f a (f b (foldr f acc list))
