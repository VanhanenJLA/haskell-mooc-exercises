...
allValues condition (Node n l r) =
  condition n
...
cull val (Node n l r)
  | n == val = Empty
  | otherwise = Node n (cull val l) (cull val r)
cull _ Empty = Empty
...
set path val tree = case walk path tree of
  Just _ -> set' path val tree
  Nothing -> tree

set' :: [Step] -> a -> Tree a -> Tree a
set' [] val (Node _ l r) = Node val l r
set' (StepL : steps) val (Node n l r) = Node n (set' steps val l) r
set' (StepR : steps) val (Node n l r) = Node n l (set' steps val r)
set' _ _ Empty = Empty
...
--
-- You may assume the value occurs in the tree once at most.
--
-- Examples:
