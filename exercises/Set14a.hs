...
greetText name =
  if T.length name > 15
    then T.concat [p' "Hello, ", T.take 15 name, p' "...!"]
    else T.concat [p' "Hello, ", name, p' "!"]
...
shout :: T.Text -> T.Text
shout = T.unwords . zipWith (curry f) [0 ..] . T.words
  where
...
-- longestRepeat = maximum . map T.length . T.group

longestRepeat xs
  | T.null xs = 0
  | otherwise = maximum $ map T.length $ T.group xs
...
takeStrict = TL.toStrict .: TL.take

(.:) :: (TL.Text -> T.Text) -> (Int64 -> TL.Text -> TL.Text) -> Int64 -> TL.Text -> T.Text
(.:) = (.) . (.)
...
countUtf8Chars a
  | B.null a = Just 0
  | otherwise = case decodeUtf8' a of
    Left _ -> Nothing
    Right b -> Just $ T.length b
...
pingpong = BL.cycle . BL.fromChunks . f
  where
    f bs = [bs, B.reverse bs]
