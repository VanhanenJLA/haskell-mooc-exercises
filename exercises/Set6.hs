...
instance Eq RationalNumber where
  p == q = (numerator p * denominator q) == (numerator q * denominator p)

numerator :: RationalNumber -> Integer
numerator (RationalNumber n _) = n

denominator :: RationalNumber -> Integer
denominator (RationalNumber _ d) = d
...
simplify p = RationalNumber (numerator p `div` g) (denominator p `div` g)
  where
    g = gcd (numerator p) (denominator p)
...
instance Num RationalNumber where
  p + q = simplify $ RationalNumber (numerator p * denominator q + numerator q * denominator p) (denominator p * denominator q)
  p * q = simplify $ RationalNumber (numerator p * numerator q) (denominator p * denominator q)
  abs q = RationalNumber (abs $ numerator q) (abs $ denominator q)
  signum q = RationalNumber (signum $ numerator q) (signum $ denominator q)
  fromInteger x = RationalNumber x 1
  negate q = RationalNumber (negate $ numerator q) (denominator q)
