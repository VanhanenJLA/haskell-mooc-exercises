...
paint (Picture c) (Shape s) (Picture b) = Picture f
  where
    f coord
      | s coord = c coord
      | otherwise = b coord
...
instance Transform Zoom where
  apply (Zoom z) (Picture f) = Picture g
    where
      g = f . zoomCoord z
...
instance Transform Blur where
  apply Blur (Picture p) = Picture p'
    where
      p' :: Coord -> Color
      p' (Coord x y) = Color r g b
        where
          r = div (sum (map getRed coords)) 5
          g = div (sum (map getGreen coords)) 5
          b = div (sum (map getBlue coords)) 5
          coords =
            [ p (Coord x y),
              p (Coord (x + 1) y),
              p (Coord (x - 1) y),
...
instance Transform BlurMany where
  apply (BlurMany n) p = foldr ($) p (replicate n applyBlur)
    where
