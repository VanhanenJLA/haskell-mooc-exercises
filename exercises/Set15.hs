...
calculator op num = case op of
  "negate" -> negate <$> readMaybe num
  "double" -> (* 2) <$> readMaybe num
  _ -> Nothing
...
validateStreetName :: Foldable t => t a -> Validation (t a)
validateStreetName x = check (length x <= 20) "Invalid street name" x

validateStreetNumber :: String -> Validation String
validateStreetNumber x = check (all isDigit x) "Invalid street number" x

validatePostCode :: [Char] -> Validation [Char]
validatePostCode x = check (length x == 5 && all isDigit x) "Invalid postcode" x
...
boolOrInt x = (Left <$> validateBool x) <|> (Right <$> validateInt x)

validateBool :: String -> Validation Bool
validateBool x = check (x == "True" || x == "False") "Not a Bool" $ read x

validateInt :: String -> Validation Int
validateInt x = check (isJust $ readMaybe x) "Not an Int" $ read x
...
normalizePhone = validatePhone . trim

trim :: String -> String
trim = filter (/= ' ')

validatePhone :: String -> Validation String
validatePhone x = check (length x <= 10) "Too long" x *> validatePhone' x

validatePhone' :: String -> Validation String
validatePhone' = foldr (\x -> (<*>) ((:) <$> validateDigit x)) (pure [])

validateDigit :: Char -> Validation Char
validateDigit x = check (isDigit x) ("Invalid character: " ++ [x]) x
...
parseExpression input = case words input of
  [arg1, op, arg2] -> parseOp op <*> parseArg arg1 <*> parseArg arg2
  _ -> invalid $ "Invalid expression: " ++ input

parseArg :: String -> Validation Arg
parseArg a = parseNum a <|> parseVar a

parseNum :: String -> Validation Arg
parseNum n =
  case (readMaybe n :: Maybe Int) of
    Just n -> check True "" (Number n)
    Nothing -> invalid $ "Invalid number: " ++ n

parseVar :: [Char] -> Validation Arg
parseVar v =
  check
    (length v == 1 && isAlpha (head v))
    ("Invalid variable: " ++ v)
    (Variable $ head v)

parseOp :: [Char] -> Validation (Arg -> Arg -> Expression)
parseOp "+" = check True "" Plus
parseOp "-" = check True "" Minus
...
instance Applicative Priced where
  pure x = Priced 0 x
  liftA2 f (Priced x y) (Priced x' y') = Priced (x + x') (f y y')
