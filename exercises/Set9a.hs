...
sumSuccess xs = case filter isRight xs of
  [] -> Left "no data"
  ys -> Right $ sum $ map (\(Right x) -> x) ys
  where
...
data Lock = LockUndefined | LockOpen String | LockClosed String deriving (Show, Eq)
...
open code (LockClosed c)
  | code == c = LockOpen c
  | otherwise = LockClosed c
open _ l = l

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (LockOpen c) = LockClosed c
lock l = l

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode code (LockOpen _) = LockOpen code
changeCode _ l = l
...
compose _ [] = []
compose xs ys =
  map (\(a, b) -> (a, fromJust $ lookup b ys)) $
    filter (\(a, b) -> isJust $ lookup b ys) xs
