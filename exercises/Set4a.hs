...
longest = foldl1 compare'

compare' :: Ord a => [a] -> [a] -> [a]
compare' this that
    | length this > length that = this
    | length this == length that =
         if head this > head that 
            then that 
            else this
    | otherwise = that
...
winner scores p1 p2
    | winner' p1 scores >= winner' p2 scores = p1
    | otherwise = p2

winner' :: String -> Map.Map String Int -> Int
winner' = Map.findWithDefault 0
...
transfer from to amount bank
    ...
    | otherwise = transfer' from to amount bank
        where
            fromBalance = Data.Maybe.fromMaybe 0 (Map.lookup from bank)

transfer' :: (Ord k, Num a) => k -> k -> a -> Map.Map k a -> Map.Map k a
transfer' from to amount bank = Map.alter credit to $ Map.alter debit from bank
    where
        credit (Just x) = Just (x + amount)
        credit _ = Just amount
