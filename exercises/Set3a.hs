...
joinToLength _ [] = []
joinToLength 0 _ = []
joinToLength limit xs = [combined | left <- xs, right <- xs, let combined = left ++ right, length combined == limit]
...
i' (c : cs) x y =
  case c of
    "printX" -> show x : (i' cs x y)
    "printY" -> (show y) : (i' cs x y)
    "left" -> i' cs (x - 1) y
    "right" -> i' cs (x + 1) y
    "up" -> i' cs x (y + 1)
    "down" -> i' cs x (y - 1)
