...
sums' i sum n
  | i == n = [sum]
  | otherwise = sum : sums' (i + 1) (sum + i + 1) n
...
indexDefault xs i def = case xs of
  [] -> def
  (x : xs)
    | i == 0 -> x
...
mymaximum bigger initial xs
  | null' xs = initial
  | otherwise = foldl' (\acc x -> if bigger acc x then acc else x) initial xs
