...
checkNumber (a, b) = do
  if any isNumber a || any isNumber b
    then Nothing
    else Just (a, b)
...
checkCapitals (for, sur) = do
  guard (isUpper (head for))
  guard (isUpper (head sur))
  return (for, sur)
...
winner scores player1 player2
  | score1 == -1 || score2 == -1 = Nothing
  | score1 > score2 = Just player1
  | score1 < score2 = Just player2
  | otherwise = Just player1
  where
    score1 = fromMaybe 0 (lookup player1 scores)
    score2 = fromMaybe 0 (lookup player2 scores)

fromMaybe :: Num p1 => p2 -> Maybe p1 -> p1
fromMaybe _ (Just x) = x
fromMaybe x Nothing = -1
...
selectSum :: (Eq a, Num a) => [a] -> [Int] -> Maybe a
selectSum xs is
  | Nothing `elem` xs' = Nothing
  | otherwise = Just (sum (map fromJust xs'))
  where
    xs' = map (safeIndex xs) is
    fromJust (Just x) = x
...
countAndLog p xs = Logger (map show (filter p xs)) (length (filter p xs))
...
balance accountName = BankOp (\x -> (getBalance x, x))
  where
    getBalance (Bank b) = Map.findWithDefault 0 accountName b
...
parensMatch s = runState (mapM_ paren s) 0 == ((), 0)
...
count x = modify (updateList x)

updateList :: Eq a => a -> [(a, Int)] -> [(a, Int)]
updateList x [] = [(x, 1)]
updateList x ((y, n) : xs)
  | x == y = (y, n + 1) : xs
  | otherwise = (y, n) : updateList x xs
